...
def _benchmark_provenance_html() -> str:
    ...
    return (
        ...
        "<p style='margin:0.3em 0;'><b>The ranking is a cross-engine gate, then "
        "<code>consensus_iptm_mean</code>.</b> Numbers below are macro-averaged AUC "
        "(binder vs non-binder discrimination) for that shipped metric. How well it separates "
        "binders depends far more on the pool than on the metric.</p>"
        ...
        "<th style='text-align:left;padding:2px 12px 2px 0;'>Macro AUC</th></tr>"
        "<tr><td style='padding:2px 12px 2px 0;'><b>Cao 2022</b> near-miss minibinders</td>"
        "<td>4,442</td>"
        "<td>12</td>"
        "<td><b>0.556</b> — 0.73 on label-clean positives</td></tr>"
        ...
        "<td><b>0.722</b></td></tr>"
        "<tr><td style='padding:2px 12px 2px 0;'><b>de novo</b> (BindCraft, Nature 2025)</td>"
        "<td>110</td>"
        "<td>7</td>"
        "<td><b>0.684</b> (0.773 on the 2 powered targets)</td></tr>"
        "</table>"
        "<p style='margin:0.3em 0;'><b>Read the top-N as a triage filter, not a decision "
        "procedure.</b> On the Cao pool — hundreds of same-tool, same-length designs against one "
        "target, the closest analogue to a real campaign — taking the top decile is worth a macro "
        "<b>1.9×</b> enrichment (FGFR2: 8.98 → 2.13 designs per hit), but it beats a random ordering "
        "on only <b>6 of 12</b> targets, and on two of them it is worse than not ranking at all. "
        "Ranking looks stronger on easier pools; it is weakest exactly where a campaign lives.</p>"
        "<p style='margin:0.3em 0;'><b>Why Cao's 0.556 is not the metric's fault.</b> 73.4% of its "
        "&quot;binder&quot; labels come from a saturated titration (one-sided K<sub>d</sub>) and are "
        "experimentally indistinguishable from non-binders on Cao's own independent binary assay "
        "(0.073% vs 0.0023% pass rate). Restricted to two-sided positives the same metric reaches "
        "0.73.</p>"
        "<p style='margin:0.3em 0;'><b>Metric choice is closed.</b> Honest nested selection over 72 "
        "candidate metrics scores <b>0.517</b> — worse than the <b>0.555</b> of the metric already "
        "shipped (p = 0.0014, 8/12 targets). The Stage-1 screen that used to sit in front of this "
        "ranking removed 0 designs from the top decile on 12/12 targets and has been retired. "
        "Committing to a single engine is the largest available mistake: the 3-engine mean is never "
        "worst on any of the three datasets.</p>"
        "<p style='margin:0.3em 0;'><b>Transferability caveat.</b> The small benchmarks share the same 4 "
