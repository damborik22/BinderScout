    ...
    checks.append(
        {
            ...
            "detail": (
                f"{len(non_standard)} sequence(s) with non-standard characters"
                if non_standard
                else "all sequences use standard amino acids"
            ),
    ...
    checks.append(
        {
            ...
            "detail": (
                f"{len(out_of_range)} sequence(s) outside 10-2000 aa range"
                if out_of_range
                else "all sequences within 10-2000 aa"
            ),
    ...
    checks.append(
        {
            ...
            "detail": (f"{len(duplicates)} duplicate sequence(s)" if duplicates else "no duplicate sequences"),
            ...
            checks.append(
                {
                    ...
                    "detail": (
                        f"target sequence OK ({len(target_seq)} aa)"
                        if not target_invalid
                        else f"target has non-standard chars: {sorted(target_invalid)}"
                    ),
                }
            )
...
def add_parser(subparsers) -> None:
    ...
    p.add_argument("--target-seq", metavar="SEQ", help="Target protein sequence (amino acid string)")
    p.add_argument("--target-pdb", metavar="PDB", help="Target PDB file (to extract sequence for length comparison)")
    p.add_argument("--json", action="store_true", help="Output results as JSON instead of text summary")
