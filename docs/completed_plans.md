# BinderScout — Completed Plans (Archive)
...
Phase 1 created standalone adapter modules in `binderscout/tools/rfaa/` and
`binderscout/tools/pxdesign/`, install scripts in `scripts/`, and a unified scoring layer
in `binderscout/scoring/`. All behind feature flags.

Phase 2 wired these into the three existing BinderScout integration points:
...
- **Installer** (`install/install.sh`): Added `install_rfaa()` and `install_pxdesign()`
  ...
  RFAA bundles LigandMPNN in the same conda env (`binderscout_rfaa`).
...
- `install_local_conda()` downloads Miniforge3 into `BinderScout/conda/`
- `detect_conda()` rewritten: local conda → writable system conda → auto-bootstrap
- `--standalone` / `--system-conda` CLI flags
- Shortcuts write to `BinderScout/bin/` instead of `~/.local/bin/`
...
Feature flags (`BINDERSCOUT_ENABLE_RFAA`, `BINDERSCOUT_ENABLE_PXDESIGN`) were used during
...
- **Unified scoring**: `binderscout.scoring.unified.BinderScore` composite formula
