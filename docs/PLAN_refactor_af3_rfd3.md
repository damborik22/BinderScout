...
**Hybrid workflow:** run `binderscout configure` + `run_all.sh` on x86, `rsync runs/<name>/ spark:~/BinderScout/runs/`, run `binderscout evaluate runs/<name>` on Spark. No code change needed — just a documented recipe in the README.
...
Why Protenix: ByteDance's open-source AlphaFold3 re-implementation, already installed as part of `binderscout_pxdesign` conda env at v0.5.0. It runs comfortably on 24 GB (PXDesign uses it for its own eval on the 3090). Same AF3-architecture metrics (pTM, ipTM, PAE, pLDDT), permissive license, commercial use fine.

**Pre-implementation research (half-day):** audit how PXDesign currently invokes Protenix — find the Python entry point, output format, required config shape. The `binderscout_pxdesign` env already has everything we need; we just need to write a thin refolder wrapper.

**New files:**
- `Evaluator/envs/binder-eval-protenix.yml` — or more likely, reuse `binderscout_pxdesign` directly (that env already has Protenix + CUDA + deps)
...
**Acceptance:** `binder-compare refold-protenix --sequences seqs.fasta --target-seq SEQ -o protenix.csv` produces a CSV with all `protenix_*` columns. `binderscout evaluate` picks it up and includes Protenix + Boltz-2 in the agreement count.
...
3. User runs `binderscout install --tool af3 --af3-weights <path>` on Spark — installer validates + stores path in `~/.binderscout/af3_weights_path`
...
**Acceptance (on Spark):**
- `binderscout install --tool af3 --af3-weights /data/af3` completes + smoke tests `run_alphafold.py --help`
...
Clone `yehlincho/Protein-Hunter@d4bd951` into `BinderScout/Protein-Hunter/`. Conda env `binderscout_protein_hunter` (Py 3.10), upstream `setup.sh` does conda + pip.

**Weight reuse:**
- Symlink `BinderScout/LigandMPNN/model_params/` → `Protein-Hunter/LigandMPNN/model_params/`
- Boltz-2 cache path = `~/.boltz/` (same dir Mosaic populates — no duplicate download)
- Chai-1 weights download fresh on first use (~5 GB)

**Shortcut:** wrapper script `BinderScout/bin/protein-hunter` activates the env and shells to `python $REPO/boltz_ph/design.py "$@"` (default) or `--chai` flag switches to `chai_ph/design.py`.
...
**License gotcha:** upstream root `LICENSE` file is missing despite README claiming MIT. Open an upstream issue/PR to add it before releasing BinderScout with this integration.

**Acceptance:**
- `binderscout install --tool protein-hunter` succeeds on x86
...
- Remove `rf_diffusion_all_atom/` + `LigandMPNN/` clones on next install (`binderscout install --uninstall --tool rfaa` still works for existing users)
...
**Add `docs/rfaa_manual_reinstall.md`** — pin commit SHAs, patch list (`idealize_backbone.py`, `residue_constants.py np.int → np.int64`), clone + conda env commands. For users who have old `runs/` and want to reproduce without BinderScout orchestration.

**Install RFD3:**
- PyPI install: `pip install "rc-foundry[rfd3]"` (pinned `v0.1.9`)
- Conda env `binderscout_rfd3` (Py 3.12, PyTorch ≥2.2 + CUDA)
- `foundry install rfd3 --checkpoint-dir BinderScout/weights/foundry` downloads weights
- **aarch64: works!** No DGL dependency, so the Grace-Hopper blocker is gone. Enable in `install_aarch.sh` for the first time.

**Shortcut:** `BinderScout/bin/rfd3` → activates env + shells to `rfd3 "$@"` (Hydra entry point).
...
**Acceptance:**
- `binderscout install --tool rfd3` on x86 and aarch64 (**first time**)
...
- `binderscout install --uninstall --tool rfaa` removes the legacy env
...
# On x86 dev box:
binderscout configure  # target.pdb, enabled tools, no AF3
bash runs/<name>/run_all.sh
rsync -av runs/<name>/ spark:~/BinderScout/runs/<name>/

# On DGX Spark:
binderscout evaluate runs/<name>  # Boltz-2 + Protenix + AF3 (3-engine agreement)
rsync -av spark:~/BinderScout/runs/<name>/report/ ./runs/<name>/report/
```
...
| Part | Smoke test |
|---|---|
| I | `binderscout evaluate runs/smoke_pdl1 --metric ipsae_min --top 5` → Boltz-2-only report, no AF2 cols |
| J | `binder-compare refold-protenix --sequences 3_seqs.fasta --target-seq SEQ -o protenix.csv` — all `protenix_*` cols |
| K | (on Spark, after weights) `binder-compare refold-af3 --sequences 3_seqs.fasta --target-seq SEQ --weights-dir $BINDERSCOUT_AF3_WEIGHTS -o af3.csv` |
| L | Per modality: `conda run -n binderscout_protein_hunter python Protein-Hunter/boltz_ph/design.py --num_designs 2 --num_cycles 2 --protein_seqs <PDL1> ...` |
...
1. On x86: clean install, then `binderscout configure` with PDL1, enabling BindCraft, BoltzGen, Mosaic, PXDesign, Proteina-Complexa, Protein Hunter (all 6 modalities across 6 runs), RFD3. Skip BindCraft AF2 eval for speed.
...
5. On Spark: re-run `binderscout evaluate runs/PDL1_all` — `agreement_count ∈ {0, 1, 2, 3}`, AF3 columns present
