...
class RFAAInferenceConfig(BaseModel):
    ...
    def ligand_must_be_three_letters(cls, v):
        if v is not None and len(v) != 3:
            raise ValueError(f"Ligand CCD code must be exactly 3 letters (e.g. 'OQO'), got: '{v}'")
...
class RFAAContigConfig(BaseModel):
    ...
    length: str | None = Field(None, description="Optional override for total length, e.g. '150-150'")
...
class RFAAConfig(BaseModel):
    ...
    rfaa_root: Path | None = Field(
        None, description="Path to rf_diffusion_all_atom clone. Defaults to BINDMASTER_RFAA_ROOT env var."
    )
    use_apptainer: bool = Field(False, description="Use Apptainer container instead of native conda env")
    apptainer_sif: Path | None = Field(
        None, description="Path to rf_se3_diffusion.sif (required if use_apptainer=True)"
    )
