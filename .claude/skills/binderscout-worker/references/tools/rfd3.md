...
**Env:** conda env `binderscout_rfd3` (replaces legacy `binderscout_rfaa`)
**Run-script template:** `binderscout_examples/run_rfd3.sh.template`
**Engine reference:** `binderscout-orchestrator/references/tools/rfd3.md`

RFD3 is in active integration in BinderScout — the configurator does not yet auto-generate run scripts. Use the template carefully; it encodes several non-obvious requirements.
...
**Note about cross-validation:** RFD3 + MPNN are independent of Boltz-2, AF3, and ESMFold2 — RFD3 outputs are among the cleanest signals in the BinderScout pool for cross-engine refold consensus. See `binderscout-orchestrator/references/tools/README.md`.
