...
**Run-script template:** `binderscout_examples/run_mosaic.sh.template`
**Engine reference:** `binderscout-orchestrator/references/tools/mosaic.md`
...
**Note about cross-validation:** Mosaic's design-time confidence is from Boltz-2 — the same model the evaluator uses by default. Mosaic-output `agreement_count` is most decisive when AF3 and ESMFold2 agree independently. See `binderscout-orchestrator/references/tools/README.md`.
