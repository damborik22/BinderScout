...
For engine principles, knobs, and "when to pick this tool" — see `binderscout-orchestrator/references/tools/<tool>.md`. The two `tools/` folders complement each other:

| Layer | Audience | Content |
|---|---|---|
| `binderscout-orchestrator/references/tools/` | orchestrator | engine, principle, native metrics, when to pick, cross-method bias |
| `binderscout-worker/references/tools/` (this folder) | worker | where progress shows up, common errors, OOM thresholds, packaging |
...
| `protein-hunter.md` | Protein-Hunter | conda `binderscout_protein_hunter` |
| `pxdesign.md` | PXDesign | conda `binderscout_pxdesign` |
| `proteina-complexa.md` | Proteina-Complexa | uv venv `Proteina-Complexa/.venv` |
| `rfd3.md` | RFD3 | conda `binderscout_rfd3` |
...
**Run-script template:** path under binderscout_examples/
