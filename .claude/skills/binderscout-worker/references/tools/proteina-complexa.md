...
**Run-script template:** `binderscout_examples/run_proteina_complexa.sh.template`
**Engine reference:** `binderscout-orchestrator/references/tools/proteina-complexa.md`

PC has the most complex setup of any tool in the BinderScout stack (Hydra config tree, multiple checkpoint variants, separate `.env` for community model paths). Verify carefully.
...
**Note about cross-validation:** Proteina-Complexa uses AF2 and RF3 as reward models internally, not Boltz-2 — so the orchestrator's Boltz-2 refold is a structurally independent check for PC outputs. See `binderscout-orchestrator/references/tools/README.md` cross-method bias matrix.
