...
**Run-script template:** `binderscout_examples/run_boltzgen.sh.template`
**Engine reference:** `binderscout-orchestrator/references/tools/boltzgen.md`
...
**Note about cross-validation:** the orchestrator knows BoltzGen's internal refolding is via Boltz-2 — same model as the evaluator's default refold. The `agreement_count` signal for BoltzGen outputs is most decisive when AF3 and ESMFold2 also agree (engines independent of BoltzGen's design lineage). See `binderscout-orchestrator/references/tools/README.md` cross-method bias matrix.
