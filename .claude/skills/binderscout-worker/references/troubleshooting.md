...
The canonical run-script templates at `binderscout_examples/run_*.sh.template` already do this. If you're hitting this, you're hand-writing a run script — don't. Use the template.
...
The `binderscout_examples/run_rfd3.sh.template` sets this. If you hand-rolled a script, add it.
...
**Cause:** the BinderScout `install/install.sh` applies patches to `protenix` (CUDA arch for Blackwell sm_120), `pxdbench` (`NumpyEncoder` JSON serialization), and `configs_infer.py` (num_workers). These are reapplied on each install but lost if you `pip install --upgrade` manually.

**Fix:** rerun `binderscout install --tool pxdesign` to reapply the patches. There is no `--pxdesign` flag; the installer selects tools with `--tool <name>`. The re-run resumes into the existing `binderscout_pxdesign` env rather than aborting, so only the patch steps redo any work.
...
**Fix:** BinderScout's `install/install.sh` was updated in commit `7642942` to use the new name. Fresh manual envs need to call `install_pyrosetta` (not `download_pyrosetta`).
...
- Hosts the AF3 v3.0.2 refolder (`binder-eval-af3` env), the Boltz-2 refolder (`Mosaic/.venv`), and the Protenix refolder (`binderscout_pxdesign`) for the orchestrator's local cross-engine refold. If Spark is also doing worker duty, be careful not to clash refolder runs with worker design runs on the same GPU.
...
3. Check `binderscout-orchestrator/references/learnings.md` §6 and §7 for env trap and log-location lessons.
4. Check the BinderScout repo's `CLAUDE.md` `Known issues` section.
