...
- **Slow on aarch64.** Measured on BM5 / GB10, single worker, 60 aa binder against a 115-residue target: **~7.4 min per trajectory**. On a hard target, where the method is expected to need many attempts per accepted design, that is days of card time per design. Prefer the x86 boxes for production.
