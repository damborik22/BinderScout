...
**Role:** design
**Status:** stable (the well-tuned reference in BinderScout)
...
- **Internal cross-validation.** AF2-ptm ↔ AF2-multimer hand-off catches designs that overfit one model. No other tool in the BinderScout stack does this natively.
...
- **iPTM is not an affinity proxy.** BindCraft itself notes iPTM is a binary binding predictor, not an affinity predictor (and the BinderScout CLAUDE.md flags iPTM as gameable across tools). BindCraft's internal iPTM ranking is its native within-tool signal; for cross-method comparison in the pooled BinderScout pool, the evaluator adds `ipsae_min` as the common comparator.
...
- You want internal AF2 cross-validation as a first-pass filter before the BinderScout evaluator.
...
- BinderScout note: iPTM is flagged gameable across tools in CLAUDE.md `Critical domain facts`; the evaluator's `ipsae_min` is the cross-method comparator.
