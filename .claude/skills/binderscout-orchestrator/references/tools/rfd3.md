...
**Role:** design
**Status:** in active integration in BinderScout (replaces deprecated RFAA; configurator does not yet generate RFD3 run scripts — written by hand from `binderscout_examples/run_rfd3.sh.template`)
**Environment:** conda env `binderscout_rfd3` (replaces legacy `binderscout_rfaa`)
...
- **Atom-level conditioning.** Specify H-bond donors/acceptors, burial states, catalytic geometry, ligand contacts — RFD3 is the only design tool in the BinderScout stack with native atomic-level constraint conditioning.
...
- **No configurator support yet in BinderScout.** Run scripts written by hand. The template at `binderscout_examples/run_rfd3.sh.template` is the canonical reference.
...
- Symmetric oligomer design — RFD3 is the only native option in the BinderScout stack.
...
- You need a turnkey configurator-managed run today — BinderScout's configurator doesn't generate RFD3 scripts yet; use the template by hand.
...
- aarch64 — should be fine architecturally (no DGL dependency like RFAA had), but BinderScout integration not yet tested on Blackwell.
