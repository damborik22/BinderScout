...
**Role:** refolding (primary in BinderScout); also used internally at design time by Mosaic, BoltzGen, and Protein-Hunter
**Status:** stable; the BinderScout default refolder, currently active on all platforms
...
- **Convenient defaults.** pLDDT on [0,1], PAE in raw Angstroms, native `[binder|target]` chain ordering — matches BinderScout's evaluator conventions without transposition or rescaling.
...
- pLDDT array — **[0,1] scale**, matches BinderScout convention
...
**iPSAE outputs (computed by BinderScout evaluator from PAE matrix):**
...
| `diffusion_samples` | 1–25 | Multiple samples → average over diffusion uncertainty. BinderScout default is 1 for refolding. |
