...
**Engine:** Iterative structure-hallucination-within-diffusion framework — uses AF3-style structure predictors (Boltz-1/2 or Chai-1) as the hallucination oracle. Lightweight, fine-tuning-free; starts from an all-X (unknown amino acid) sequence and iteratively cycles structure prediction → sequence sampling until convergence. Two backends in the repo: `boltz_ph` (Boltz edition, used by BinderScout) and `chai_ph` (Chai edition, under active development). Optional AF3 cross-validation.
**Role:** design
**Status:** in active integration (no automated configurator yet in BinderScout; run scripts written by hand from `binderscout_examples/run_protein_hunter.sh.template`)
**Environment:** conda env `binderscout_protein_hunter`; requires Boltz-2 cache populated at `~/.boltz/`
...
- **Experimental software, no automated runner.** Per BinderScout CLAUDE.md, the configurator does not yet generate Protein-Hunter run scripts; written by hand from the template.
...
- **CLAUDE.md runtime gotchas** — each one bit during the CALCA run:
  ...
  - `pyrosetta-installer` ≥ 0.3 renamed `download_pyrosetta` → `install_pyrosetta`. BinderScout's `install/install.sh` was updated in commit `7642942`; fresh manual envs need the new name.
...
- **AF3 cross-validation is heavy.** Requires AF3 Docker, database paths, HMMER. Off by default in BinderScout usage.
...
- You need a turnkey, configurator-managed run — BinderScout doesn't auto-generate run scripts yet.
- You need stable cross-model validation — same-model self-judging applies (the Boltz-2 used for design is the same one used by the BinderScout evaluator's default refold).
...
| `--use_msa_for_af3` | off (BinderScout default) | Use MSA when AF3 cross-validation enabled. |
| `--use_alphafold3_validation` | off (BinderScout default) | Enable AF3 cross-validation. Requires AF3 Docker, database, HMMER. |
