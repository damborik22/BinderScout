...
Because hallucination uses the predictor *itself* as the design oracle, design-time confidence overstates real binding likelihood. Cross-validation must come from the BinderScout evaluator (Boltz-2 + AF3 + ESMFold2 refold), not from Mosaic itself.

## Strengths

- **Composable objectives.** Combine binder + monomer stability + off-target avoidance + ESM/MPNN priors + custom terms in one optimization. No other tool in the BinderScout stack does this.
...
- iPSAE formula reference: DunbrackLab 2025 (`d0_res` variant, uniform 10 Å PAE cutoff in BinderScout evaluator)
