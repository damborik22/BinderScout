...
- `docs/local/BinderScout-Slurm-Orchestration-Manual.md` — per-tool recipes + gotchas
...
the sibling `binderscout-worker` skill for the generic worker mechanics.
...
ssh clara "ls ~/miniforge3/envs/<env> 2>/dev/null || ls ~/BinderScout/<Tool>/.venv"  # env present
...
ssh clara "cd ~/BinderScout && git rev-parse --short HEAD"      # repo at the SHA you expect
```
