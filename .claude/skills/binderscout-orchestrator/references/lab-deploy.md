...
handoff-doc model (SKILL.md §4 / the `binderscout-worker` skill).
...
present, BinderScout git SHA/branch, tmux version, reachability, timestamp.
...
- **BindCraft 2 on BM1 lives in a git worktree on an unpushed branch — you
  ...
  BinderScout SHA/branch per machine and it reads the **main** checkout — so
...
- **BM5 is aarch64 — Protein-Hunter is untested there. But *not* because of
  ...
  it, and BinderScout has been using that route on this machine all along.
  ...
  **What remains true, and is untested rather than known:** Protein-Hunter's
  ...
  One specific trap: a `binderscout_protein_hunter` conda env **does exist on
...
BinderScout git SHA/branch, tmux version, and writes
...
### 4bis.3 Corollary — after syncing a machine's BinderScout repo, patch Mosaic
...
BinderScout without applying the patch makes every design fail its feature
build:
