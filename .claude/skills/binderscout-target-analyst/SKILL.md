...
**When you read this:** at the very start of a campaign, before `binderscout-orchestrator`
...
- Not a design runner (that's `binderscout-worker`) or an allocator (that's `binderscout-orchestrator`).
...
2. **Build the MSA** — reuse the repo's `get_target_msa(seq)`
   ...
   returns an A3M, **cached** (`~/.cache/binderscout/target_msa/`). The MSA gives:
...
→ **`binderscout-orchestrator`**: the dossier's campaign params seed the per-tool assignments.
...
- Sibling skills: `binderscout-orchestrator` (consumes the dossier), `binderscout-evaluator`, `binderscout-wetlab`.
