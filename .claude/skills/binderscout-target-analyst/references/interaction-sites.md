...
`binderscout-wetlab` and `binderscout-evaluator` (`qc.md`), not target analysis.
...
3. **Pick hotspots (3–6 residues) on ONE contiguous patch.** Prefer residues that line the chosen
   ...
   face**. Hard rule from `binderscout-orchestrator/references/learnings.md` #2: hotspots scattered
