...
# BinderScout Wet-Lab Advisor — SKILL base
...
## 3. Handoff
→ **`binderscout-orchestrator`**: the `mature` spec (strategy + parent ids) seeds a new DESIGNING
...
- Siblings: `binderscout-evaluator` (supplies the shortlist), `binderscout-orchestrator` (runs the round).
