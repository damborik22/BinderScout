...
def write_run_all(path: Path, cfg: dict, tools_enabled: dict):
    ...
    lines = [
        "#!/usr/bin/env bash",
        f"# Run all enabled tools for {cfg['name']} in order: Mosaic → BoltzGen → BindCraft → RFAA → PXDesign → Proteina-Complexa → Evaluator",
        "# Generated by BindMaster Configurator",
        "set -euo pipefail",
        "",
        f'RUN_DIR="{run_dir}"',
...
complexa design configs/search_binder_local_pipeline.yaml \\
    ...
    ++generation.filter.filter_samples_limit={n_designs} \\
