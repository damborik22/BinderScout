...
# Per-residue sidechain atoms used to build RFD3 atom_level_hotspots.
# RFD3 expects 2 representative sidechain atoms per hotspot (matches bcov examples).
RFD3_HOTSPOT_ATOMS = {
    "ALA": "CB,CB",
    "ARG": "CG,CZ",
    "ASN": "CG,ND2",
...
def _rfd3_hotspot_block(target_pdb: Path, chain: str, hotspot_residues: list[int]) -> str:
    """Build the atom_level_hotspots YAML block for an RFD3 inputs file.

    Looks up each hotspot residue's amino-acid type in target_pdb to pick
    representative sidechain atoms.  RFD3 uses chain `B` for the target by
    convention (binder is `A`), so the chain id used in the YAML is always `B`.
    """
    ...
    try:
        for line in target_pdb.read_text().splitlines():
            if line.startswith("ATOM") and line[12:16].strip() == "CA" and line[21] == chain:
                res_type[int(line[22:26])] = line[17:20].strip().upper()
    except OSError:
        pass

    if not hotspot_residues:
        return "    atom_level_hotspots: {}"

    lines = ["    atom_level_hotspots:"]
    for r in hotspot_residues:
        atoms = RFD3_HOTSPOT_ATOMS.get(res_type.get(r, ""), "CB,CB")
        lines.append(f"      B{r}:")
...
def write_run_rfd3(path: Path, cfg: dict):
    """Generate run_rfd3.sh — protein-protein binder design with RFdiffusion3 (foundry)."""
    run_dir = cfg["run_dir"]
    ...
    diffusion_steps = cfg.get("rfd3_diffusion_steps", 100)

    hotspots_str = cfg.get("rfd3_hotspots") or cfg.get("hotspots", "") or ""
    hotspot_list = parse_hotspots(hotspots_str) if hotspots_str.strip() else []
    hotspot_block = _rfd3_hotspot_block(target_pdb, primary_chain, hotspot_list)

    name = cfg["name"]
    total_min = min_len + target_len
    total_max = max_len + target_len

    content = f"""\
#!/usr/bin/env bash
# Run RFdiffusion3 (foundry) for {name}
# Generated by BindMaster Configurator
set -euo pipefail

RUN_DIR="{run_dir}"
RFD3_DIR="$RUN_DIR/rfd3"
TARGET_SRC="{target_pdb}"
TARGET_PDB="$RFD3_DIR/target_chainB.pdb"
INPUTS_YAML="$RFD3_DIR/inputs.yaml"
OUT_DIR="$RFD3_DIR/outputs"

mkdir -p "$RFD3_DIR" "$OUT_DIR"

# RFD3 conventions assume binder=A, target=B.  Rename the target chain {primary_chain} -> B.
python3 - "$TARGET_SRC" "$TARGET_PDB" <<'PYEOF'
import sys
...
# Build the inputs YAML (one target).
...
{name}:
    input: $TARGET_PDB
    contig: {min_len}-{max_len},/0,B1-{target_len}
    contig_atoms: '{{}}'
    length: {total_min}-{total_max}
    redesign_motif_sidechains: false
{hotspot_block}
INPUTSEOF

# foundry checkpoint discovery uses FOUNDRY_CHECKPOINT_DIRS (plural).
...
echo ""
echo "=== Collecting RFD3 binder sequences ==="
python3 - "$OUT_DIR" "$RFD3_DIR/sequences.csv" "{name}" <<'PYEOF'
import csv, sys
from pathlib import Path
...
for pdb in sorted(out_dir.rglob("*.pdb")):
    ...
    with open(pdb) as f:
        for line in f:
            if line[:4] != "ATOM" or line[12:16].strip() != "CA":
                continue
            if line[21] != "A":  # binder is chain A
                continue
            key = (line[22:26].strip(), line[26].strip())
            if key not in seen:
    ...
    if seq and set(seq) != {{"X"}}:
        rows.append({{
            "design_id": f"rfd3_{{pdb.stem}}",
            "sequence": seq,
            "length": len(seq),
            "source": "rfd3",
        }})
if not rows:
    print("WARNING: no RFD3 binder sequences found", file=sys.stderr)
    sys.exit(0)
with open(csv_path, "w", newline="") as f:
    w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
    w.writeheader()
    w.writerows(rows)
print(f"  -> {{len(rows)}} sequences written to {{csv_path}}")
PYEOF
"""
    path.write_text(content)
    path.chmod(0o755)
