...
def detect_installs() -> dict:
    """
    BindCraft / BoltzGen: check for their conda env directory OR local venv.
    ...
    def _has_venv(tool_dir: Path) -> bool:
...
def banner():
    ...
    if CONDA_BASE:
        ...
        # Check if any tools use local venvs (venv mode)
        has_venv_tools = any((d / ".venv" / "bin" / "python").exists() for d in [BINDCRAFT_DIR, BOLTZGEN_DIR])
        if has_venv_tools:
            print(f"  {CYAN}Conda base{RESET}: not found (tools installed via venv)")
        else:
            print(f"  {CYAN}Conda base{RESET}: {RED}not found — install Miniconda/Mambaforge{RESET}")
    print()
    if CONDA_BASE is None and not any((d / ".venv" / "bin" / "python").exists() for d in [BINDCRAFT_DIR, BOLTZGEN_DIR]):
...
def _bindcraft_uses_venv() -> bool:
    """Check if BindCraft was installed via venv (vs conda)."""
    return (BINDCRAFT_DIR / ".venv" / "bin" / "python").exists()


def write_run_bindcraft(path: Path, cfg: dict):
    run_dir = cfg["run_dir"]

    if _bindcraft_uses_venv():
        # Venv-based activation (no conda required)
        content = f"""\
#!/usr/bin/env bash
...
"""
        ...
        # Conda-based activation (original)
