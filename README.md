...
- **Protein-Hunter**: not supported on aarch64, and `install_aarch.sh` rejects `--tool protein-hunter`. The reason it gives — "PyRosetta has no aarch64 wheels" — is wrong: PyRosetta runs here from the native aarch64 *conda* build that `install_aarch.sh` itself installs for BindCraft. What is genuinely untested is Protein-Hunter's own installer, which takes the pip-wheel path, and its vendored Boltz-2 + Chai-1 stack. Untested, not blocked.
