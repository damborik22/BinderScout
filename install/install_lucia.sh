#!/bin/bash
# BindMaster Installer — Lucia Edition
# Standalone variant that uses BindCraft_1 conda env name to coexist
# with an existing BindCraft environment that cannot be removed.
#
# Usage:
#   bash install/install_lucia.sh [--tool bindcraft|boltzgen|mosaic|all] [--cuda VERSION] [--skip-examples] [--yes]
...
LOG_FILE="${BINDMASTER_DIR}/install_lucia.log"

BINDCRAFT_DIR="${BINDMASTER_DIR}/BindCraft_1"
...
install_bindcraft() {
    ...
    if ! env_exists BindCraft_1; then
        ...
        print_warn "This will take 45-90 min — full output in ${LOG_FILE}"
        run_logged "Installing BindCraft (conda packages + AlphaFold2 weights)" \
            bash -c "cd '${BINDCRAFT_DIR}' && bash install_bindcraft.sh --cuda '${CUDA_VERSION}' --pkg_manager conda --name BindCraft_1" \
    ...
    # Shortcut
    print_step "Installing bindcraft1 shortcut"
    _write_bindcraft1_shortcut
    print_ok "Shortcut installed at ${SHORTCUTS_DIR}/bindcraft1"
...
_write_bindcraft1_shortcut() {
    ...
    {
        echo "#!/bin/bash"
        echo "# BindCraft_1 shortcut (bindcraft1) — activates the BindCraft_1 conda environment"
        echo "# and opens an interactive shell in the BindCraft_1 directory."
        echo ""
        echo "BINDCRAFT_DIR=\"${BINDCRAFT_DIR}\""
        echo "CONDA_BASE=\"${CONDA_BASE}\""
    } > "${SHORTCUTS_DIR}/bindcraft1"
    cat >> "${SHORTCUTS_DIR}/bindcraft1" << 'EOF'
...
exec bash
EOF
    chmod +x "${SHORTCUTS_DIR}/bindcraft1"
}
...
install_evaluator() {
    ...
    if ! is_mosaic_installed; then
        ...
        print_warn "Run: bash install/install_lucia.sh --tool mosaic"
        return 1
    fi
...
main() {
    ...
    [[ "${DO_BINDCRAFT}" == true ]] && echo -e "  ${GREEN}bindcraft1${RESET}  — open BindCraft shell"
