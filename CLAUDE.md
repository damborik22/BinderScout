...
| **consensus_iptm** | max ipTM across independent refolding engines. Reported as a column and behind the informational `passes_max_screen` flag; **no longer steers the ranking** (Stage-1 retired, Part U). The ranking metric is `consensus_iptm_mean` |
...
**Primary ranking: two-stage cross-engine iPTM** — `binder-compare report --rank-by two_stage` (the default). **Stage 0 (cross-engine gate):** a design must have been refolded by at least `--min-engines` independent engines (**default 3** = all of Boltz-2 / AF3 / ESMFold2; floor 2) to be eligible. `consensus_iptm_mean` skips missing engines, so without this a single-engine design's mean *is* that one engine's score and it competes against 3-engine means on an incomparable scale — and for Mosaic / Protein-Hunter that engine is the one that designed it. `consensus_iptm_n` records the count and is also a rank tiebreaker. If nothing in the pool clears the gate, the run warns and names the shortfall rather than silently reporting that no design was good enough. **Stage 1 (screen) — RETIRED 2026-07-28 (Part U).** `passes_max_screen` is still computed and reported (it marks where the top-50% cut falls and the report draws its threshold line from it) but it is **no longer a sort key**. On the Cao 2022 benchmark (4,442 designs, 12 targets, all three engines on every design) the screen removed **0 designs from the top-5/10/20/50 and top-10% on 12/12 targets**; the earliest rank it moved at all was 21.0% down the pool, and full-list Spearman between screened and unscreened orderings was 0.983–0.999. It cost a code path and changed nothing anyone reads. `--screen-metric` is deprecated and now only selects the column behind the diagnostic flag. **Stage 2 (rank):** `consensus_iptm_mean` orders the gated pool. Note the "precision@top-10% 0.92 vs 0.79" figure is evidence for **mean-ranking over max-ranking** — not for the two-stage architecture. `adaptyv_rank` (agreement_count → ipsae_min) and `consensus_rank` (max only) remain as columns but are no longer the default sort.

> **Part U caveat on max-vs-mean.** The retired max screen was documented as "the lenient recall step". Measured on Cao that is backwards: at the same 50% cut the **mean** screen retained *more* true binders (1,114 vs 1,093; +0.0123 macro recall, CI [+0.0027, +0.0226], p=0.0094, 8/12 targets). Moot now that neither steers the rank, but do not reinstate the max screen on the old rationale. The inertness is an empirical property of correlated engines (max/mean co-rank at Spearman 0.89–0.98 within target), **not** a theorem — elementwise `mean ≤ max` does not imply rank preservation.
