...
- Protein-Hunter: not supported on aarch64 — **but the stated reason was wrong**. PyRosetta does run here: BM5's `BindCraft` env imports it, runs `pyrosetta.init()` and scores a pose, from the native aarch64 *conda* build (`PyRosetta4.conda.aarch64...`, graylab JHU channel) that `install/install_aarch.sh` already installs. "No aarch64 wheels" was only ever true of PyPI. What is actually unresolved: Protein-Hunter's own installer takes the **wheel** path (`pyrosetta-installer` via pip) and nobody has tried repointing it at the conda build here, and its vendored Boltz-2 + Chai-1 stack has never been built on aarch64. So this is **untested, not blocked** — and do not upgrade "PyRosetta works" into "Protein-Hunter installs". Note also that a `bindmaster_protein_hunter` env exists on BM5 but is an empty ~198 MB shell: the env name is not evidence of an install
